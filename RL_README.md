...
## 在 HTML 里动态演示：手写 AI vs Chemiss-Entropior（无后端）

设置栏有一个 **「手写AI vs Chemiss-Entropior 演示」** 按钮（⚡ 图标），点击即开始一盘
**手写 AI(黑) vs ML Chemiss-Entropior(白)** 的自动对弈，棋盘实时动画。
...
- 说明：演示按钮（⚡ 手写 AI vs Chemiss-Entropior）**不受影响**——它明确用 `_forceML` 实现，
  `_disableML` 不影响该演示路径。
...
| `Chemiss-Entropior` | 神经网络(策略+价值) + PUCT 树搜索（`_aiModel='mcts'`，`_forceML=true`） |
...
- **人机对战的说明**：人机模式下，AI 方按模型配置走子（AI 执黑用「黑方」模型、执白用「白方」）。
  侧栏会显示 **AI模型** 一行；事件日志也会写明 `AI 黑方 用 Chemiss-Entropior / NNUE / 手写AI`。
  模型配置页的说明区同步解释了三种模型及人机/自动对弈的差异。
- 已把「手写AI vs Chemiss-Entropior 演示」(`azDemo`) 升级为按颜色的 `whiteModel/blackModel`。
